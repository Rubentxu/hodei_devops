...
type Scheduler interface {
	SelectCandidateNodes(t domain.Task, nodes []*node.Node) []*node.Node
	Score(t domain.Task, nodes []*node.Node) map[string]float64
...
func (r *RoundRobin) SelectCandidateNodes(t domain.Task, nodes []*node.Node) []*node.Node {
	return nodes
}

func (r *RoundRobin) Score(t domain.Task, nodes []*node.Node) map[string]float64 {
...
func (g *Greedy) SelectCandidateNodes(t domain.Task, nodes []*node.Node) []*node.Node {
	return selectCandidateNodes(t, nodes)
}

func (g *Greedy) Score(t domain.Task, nodes []*node.Node) map[string]float64 {
	nodeScores := make(map[string]float64)
...
func (e *Epvm) SelectCandidateNodes(t domain.Task, nodes []*node.Node) []*node.Node {
	return selectCandidateNodes(t, nodes)
}

func (e *Epvm) Score(t domain.Task, nodes []*node.Node) map[string]float64 {
	nodeScores := make(map[string]float64)
	maxJobs := 4.0
...
func selectCandidateNodes(t domain.Task, nodes []*node.Node) []*node.Node {
...
func checkDisk(t domain.Task, diskAvailable int64) bool {
