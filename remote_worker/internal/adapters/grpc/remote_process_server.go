...
type ServerAdapter struct {
	remote_worker.UnimplementedRemoteProcessServiceServer
...
func (s *ServerAdapter) StartProcess(stream remote_worker.RemoteProcessService_StartProcessServer) error {
	...
	for output := range outputChan {
		err := stream.Send(&remote_worker.ProcessOutput{
...
func (s *ServerAdapter) StopProcess(ctx context.Context, req *remote_worker.ProcessStopRequest) (*remote_worker.ProcessStopResponse, error) {
	...
	return &remote_worker.ProcessStopResponse{Success: true, Message: "Process stopped successfully"}, nil
}

// MonitorHealth implementa la lógica para monitorizar el estado de un proceso
func (s *ServerAdapter) MonitorHealth(stream remote_worker.RemoteProcessService_MonitorHealthServer) error {
	...
	for healthStatus := range healthChan {
		err := stream.Send(&remote_worker.ProcessHealthStatus{
...
func (s *ServerAdapter) CollectMetrics(stream remote_worker.RemoteProcessService_CollectMetricsServer) error {
	return s.metricsHandler.CollectMetrics(stream)
}

func (s *ServerAdapter) SyncFiles(req *remote_worker.FileSyncRequest, stream remote_worker.RemoteProcessService_SyncFilesServer) error {
	return s.syncHandler.SyncFiles(req, stream)
}

func (s *ServerAdapter) SyncFilesStream(stream remote_worker.RemoteProcessService_SyncFilesStreamServer) error {
	return s.syncHandler.SyncFilesStream(stream)
}

func (s *ServerAdapter) GetRemoteFileList(ctx context.Context, req *remote_worker.RemoteListRequest) (*remote_worker.RemoteFileList, error) {
	return s.syncHandler.GetRemoteFileList(ctx, req)
}

func (s *ServerAdapter) DeleteRemoteFile(ctx context.Context, req *remote_worker.RemoteDeleteRequest) (*remote_worker.RemoteDeleteResponse, error) {
...
func (s *ServerAdapter) Start(tlsConfig *tls.Config, authInterceptor *security.AuthInterceptor, env string) {
	...
	remote_worker.RegisterRemoteProcessServiceServer(grpcServer, s)
...
// ConvertPortsProcessStatusToProto converts ports.ProcessStatus to remote_worker.ProcessStatus
func ConvertPortsProcessStatusToProto(status ports.ProcessStatus) remote_worker.ProcessStatus {
	switch status {
	case ports.UNKNOWN:
		return remote_worker.ProcessStatus_UNKNOWN_PROCESS_STATUS
	case ports.RUNNING:
		return remote_worker.ProcessStatus_RUNNING
	case ports.HEALTHY:
		return remote_worker.ProcessStatus_HEALTHY
	case ports.ERROR:
		return remote_worker.ProcessStatus_ERROR
	case ports.STOPPED:
		return remote_worker.ProcessStatus_STOPPED
	case ports.FINISHED:
		return remote_worker.ProcessStatus_FINISHED
	default:
		return remote_worker.ProcessStatus_UNKNOWN_PROCESS_STATUS
	}
}
...
func (s *ServerAdapter) ExecuteCommand(ctx context.Context, req *remote_worker.ExecuteCommandRequest) (*remote_worker.ExecuteCommandResponse, error) {
	...
	return &remote_worker.ExecuteCommandResponse{
		Success:            success,
...
func createErrorResponse(message string, exitCode int) (*remote_worker.ExecuteCommandResponse, error) {
	errorDetails := map[string]interface{}{
...
func executeJcCommand(ctx context.Context, args []string) (*remote_worker.ExecuteCommandResponse, error) {
...
func validateAnalysisOptions(opts *remote_worker.StructuredAnalysisOptions) error {
