...
func main() {
	...
	accessibleRoles := map[string][]string{
		...
		"/remote_worker.RemoteProcessService/StopProcess":       {"admin", "operator"},
		"/remote_worker.RemoteProcessService/MonitorHealth":     {"admin", "operator", "viewer"},
		...
		"/remote_worker.RemoteProcessService/GetRemoteFileList": {"admin", "operator", "viewer"},
		"/remote_worker.RemoteProcessService/DeleteRemoteFile":  {"admin", "operator"},
	}
	...
	serverAdapter := remote_worker_server.NewAdapter(
		executor,
		metricsService,
		syncService,
		address,
