...
type RPSClient struct {
	client    remote_worker.RemoteProcessServiceClient
...
func New(cfg *RemoteProcessClientConfig) (*RPSClient, error) {
	...
	client := remote_worker.NewRemoteProcessServiceClient(conn)
	return &RPSClient{
		client:    client,
		conn:      conn,
...
func (c *RPSClient) StartProcess(
	...
	for {
		...
		// Convertir remote_worker.ProcessOutput a domain.ProcessOutput
		domainOutput := model.ProcessOutput{
			ProcessID: output.ProcessId,
			Output:    output.Output,
			IsError:   output.IsError,
			Type:      output.Type,
			Status:    model.ConvertProtoProcessStatusToPorts(output.Status),
		}
...
func (c *RPSClient) StopProcess(ctx context.Context, processID string) (bool, string, error) {
	...
	resp, err := c.client.StopProcess(ctx, &remote_worker.ProcessStopRequest{
		ProcessId: processID,
	})
	if err != nil {
...
func (c *RPSClient) MonitorHealth(ctx context.Context, processID string, checkInterval int64, healthChan chan<- *model.ProcessHealthStatus) error {
	ctx = c.createAuthContext(ctx)
	...
	err = stream.Send(&remote_worker.HealthCheckRequest{
		ProcessId:     processID,
		...
		for {
			...
			default:
				...
				if err != nil {
					...
					select {
					case healthChan <- &model.ProcessHealthStatus{
						ProcessID: processID,
						Status:    model.ERROR,
				...
				case healthChan <- &model.ProcessHealthStatus{
					ProcessID: processID,
					Status:    model.ConvertProtoProcessStatusToPorts(healthStatus.Status),
...
func (c *RPSClient) StreamMetrics(ctx context.Context, workerID string, metricTypes []string, interval int64, metricsChan chan<- *remote_worker.WorkerMetrics) error {
	ctx = c.createAuthContext(ctx)
	...
	err = stream.Send(&remote_worker.MetricsRequest{
		WorkerId:    workerID,
